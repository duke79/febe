...
from lib.py.core.copy import copytree
...
class MyEventHandler(FileSystemEventHandler):
            ...
            if os.path.exists(file_path):
                try:
                    if os.path.isfile(file_path):
                        ...
                        copytree(file_path, target_path)
                except (PermissionError, FileNotFoundError) as e:
                    print(e)
        else:
            target_path = os.path.normpath(target_path)
            target_path = target_path.replace("\\", "/")
            ret = os.system("docker cp {src_path} {container}:{target_path}".format_map({
                "src_path": file_path,
                "container": self._docker_container,
                "target_path": target_path
            }))
            if ret == 1:
                cmd = "docker exec {container} mkdir -p {target_path}".format_map({
                    "container": self._docker_container,
    ...
    def is_ignored(self, file_path):
        ...
        orginal_dir = os.path.abspath(os.curdir)
        ...
        for file in list_of_files:
            ...
            normdir = os.path.dirname(normfile)
            if normfile == file_path:
                ret = True
                break
            elif normdir.__contains__(file_path) and os.path.isdir(file_path):
                ret = True
                break
...
if __name__ == "__main__":
    ...
    dest_dir = sys.argv[2] if len(sys.argv) > 2 else 'D:\\share'
